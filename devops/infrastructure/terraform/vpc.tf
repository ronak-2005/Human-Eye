...
resource "aws_subnet" "public" {
  ...
  tags = { Name = "${var.app_name}-public-${count.index + 1}-${var.environment}" }
}
...
resource "aws_subnet" "private" {
  ...
  tags = { Name = "${var.app_name}-private-${count.index + 1}-${var.environment}" }
}
...
resource "aws_security_group" "ml_engine" {
  name        = "${var.app_name}-ml-engine-sg-${var.environment}"
  description = "ML Engine — allow from backend only (internal)"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "rds" {
  ...
  description = "RDS — allow from backend + ML engine only"
  vpc_id      = aws_vpc.main.id
  ...
  ingress {
    ...
    to_port         = 5432
    protocol        = "tcp"
    security_groups = [aws_security_group.backend.id, aws_security_group.ml_engine.id]
  }
  egress {
    from_port   = 0
    to_port     = 0
...
resource "aws_security_group" "redis" {
  ...
  description = "Redis — allow from backend + celery only"
  vpc_id      = aws_vpc.main.id
